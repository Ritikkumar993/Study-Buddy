    ...
    if example == 1:
        with st.sidebar:
            selected = option_menu(
                ...
                options=["Road Map","Code Editor","Mock Interview","AI Bot","DSA BOT","Hack Bot","Interview Bot"],  # required
...
if selected== "DSA BOT":
    ...
    with col1:
        st.lottie(l, height=100, width=100)
    with col2:
        st.header(f":rainbow[DSA BOT]💻💻", divider='rainbow')
    with st.container(border=True):  
        col1, col2 = st.columns([1,1])
...
if selected == "Interview Bot":
    ...
    with col1:
        st.lottie(l, height=100, width=100)
    with col2:
        st.header(f":rainbow[Interview Bot]💻💻", divider='rainbow')
    with st.container(border=True):
        col1, col2 = st.columns([1,1])
